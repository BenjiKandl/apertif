...
// Theme definitions for the app. Only two themes are available: a light "white"
// theme and a dark "black" theme. Each defines a gradient background and
// primary colours used for buttons and progress bars. You can add more
// themes by extending this object.
const themes = {
  white: {
    gradient: 'from-white via-gray-50 to-gray-100',
    primary: '#2563EB',      // muted cool blue accent
    light: '#EEF2FF'         // soft blue tint
  },
  black: {
    gradient: 'from-gray-800 via-gray-900 to-black',
    primary: '#2563EB',      // same accent for consistency
    light: '#1F2937'         // dark surface tint
  }
};
...
function downloadIcs(event) {
  ...
  ics += 'LOCATION:' + (event.address || event.station) + '\r\n';
  if (event.dressCode) {
...
function RsvpForm({ event, guests, capacity, addGuest, showToast, theme, eventId }) {
  const [name, setName] = useState('');
  const [selectedDiets, setSelectedDiets] = useState([]);
  const [bottle, setBottle] = useState(bottleOptions[0]);

  // Optional contact info (email or phone) for reminders/ratings
  const [contact, setContact] = useState('');

  // Track whether the guest has decided to attend. Values: 'undecided', 'yes', 'no'
  const [decision, setDecision] = useState('undecided');

  // Prevent multiple RSVPs by checking localStorage for this event
  const [alreadyRsvped, setAlreadyRsvped] = useState(false);

  useEffect(() => {
    if (eventId) {
      const key = 'apertif_rsvp_' + eventId;
      setAlreadyRsvped(!!localStorage.getItem(key));
    }
  }, [eventId]);
  ...
  const handleSubmit = (e) => {
    ...
    // Mark this event as RSVPed so the user cannot RSVP again
    if (eventId) {
      const key = 'apertif_rsvp_' + eventId;
    ...
    <div className={`min-h-screen bg-gradient-to-br ${bgGradient} bg-fixed flex flex-col items-center`}> 
      {alreadyRsvped ? (
        <div className="mt-8 p-6 bg-white/80 backdrop-blur-md rounded-xl shadow-lg w-full max-w-md space-y-4">
          <p className="text-center text-lg">You have already RSVPed to this event. Thank you!</p>
        </div>
      ) : (
        <div className="mt-8 p-6 bg-white/80 backdrop-blur-md rounded-xl shadow-lg w-full max-w-md space-y-4">
        {/* Event summary */}
        ...
        <div>
          ...
          {/* Show address with Google Maps link once the guest has agreed to attend */}
          {decision === 'yes' && event.address && (
            <p className="text-gray-700"><strong>Address:</strong> <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(event.address)}`} target="_blank" rel="noopener noreferrer" className="underline text-blue-600">{event.address}</a></p>
          )}
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-4">
              <button
                ...
                style={{ backgroundColor: themes[theme]?.primary || themes.white.primary }}
              >
                Yes, I’m in!
              </button>
              <button
        ...
        {decision === 'yes' && (
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div>
              <label className="block font-medium">Your email or phone (optional)</label>
              <input
                type="text"
                className="w-full border rounded px-3 py-2"
                value={contact}
                onChange={(e) => setContact(e.target.value)}
                placeholder="For reminders and flake rating"
              />
            </div>
            <button
              type="submit"
...
function App() {
  ...
  const createEvent = () => {
    if (!form.title || !form.date || !form.time || !form.station) {
      alert('Please fill in the dinner name, date, time and nearest tube station');
      return;
    }
  ...
  // Send reminders to all guests (placeholder). In a real app this would trigger
  // an email or SMS. Here we simply show a toast notification. If there
  // are no guests yet, alert the host.
  const remindGuests = () => {
    if (guests.length === 0) {
      showToast('There are no guests to remind yet.');
      return;
    }
    showToast('Reminders sent to guests! 📣');
  };
  ...
  const accentLight = themes[form.theme]?.light || themes.white.light;
      ...
      <div className={`min-h-screen bg-gradient-to-br ${themeBg} bg-fixed flex flex-col items-center`}> 
        <div className="mt-8 p-6 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg w-full max-w-2xl space-y-6">
          ...
          <div className="space-y-1">
            ...
            {form.address && (
              <p className="text-gray-700"><strong>Address:</strong> <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(form.address)}`} target="_blank" rel="noopener noreferrer" className="underline text-blue-600">{form.address}</a></p>
            )}
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white ${getAvatarClass(guest.name)}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 flex flex-wrap gap-2">
                        ...
                        {guest.contact && (
                          <span className="px-2 py-0.5 bg-gray-200 rounded-full text-xs">{guest.contact}</span>
                        )}
                      </div>
                    </div>
                </div>
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${themeBg} bg-fixed flex flex-col items-center`}> 
      <div className="mt-8 p-6 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg w-full max-w-2xl space-y-6">
        ...
        <h1 className="text-4xl font-bold text-center" style={{ color: themes[form.theme || 'white']?.primary || themes.white.primary }}>APERTIF beta by Benji</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block font-medium">Dinner Name</label>
            <input type="text" name="title" value={form.title} onChange={handleChange} className="w-full border rounded px-3 py-2" placeholder="e.g. Benji’s Feast" />
          </div>
          <div>
          ...
          {/* Address or postcode for map link */}
          <div>
            <label className="block font-medium">Address / Postcode</label>
            <input type="text" name="address" value={form.address} onChange={handleChange} className="w-full border rounded px-3 py-2" placeholder="Postcode or full address" />
          </div>
          <div>
          ...
          <div>
            <label className="block font-medium">You can arrive up to (minutes) late</label>
            ...
            <div className="flex items-center gap-2 mt-1">
              {['white','black'].map((th) => (
                <button
                  ...
                  onClick={() => setTheme(th)}
                  className={`px-3 py-1 rounded-full border transition-colors duration-200 ${
                    form.theme === th ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'
                  }`}
                >
